"""
Testes da camada de lógica de desafio3.py.
...
def test_salvar_como_csv(tmp_path, monkeypatch):
    ...
    nome = d.salvar_usuarios_em_arquivo(
        USUARIOS_EXEMPLO,
        "csv",
        nome_base="teste",
    )
...
def test_salvar_usuarios_em_arquivo_sem_usuarios_da_erro(
    tmp_path,
    monkeypatch,
):
    monkeypatch.chdir(tmp_path)
...
def test_salvar_usuarios_em_arquivo_formato_invalido_da_erro(
    tmp_path,
    monkeypatch,
):
    monkeypatch.chdir(tmp_path)
...
# ------------------------------------------------------------------
# buscar_usuarios_na_api
# rede mockada, sem chamar a internet de verdade
# ------------------------------------------------------------------
...
def test_buscar_usuarios_na_api_vazia_da_erro():
    ...
    with patch(
        ...
        with pytest.raises(
            ValueError,
            match="não devolveu nenhum usuário",
        ):
